...
- **Queue (in order):** ① P0 fixes ✅ → ② fractional positioning ✅ → ③ REST features ✅ → ④ WebSocket + participation broadcast ← *next* → ⑤ post-Phase-1 integrity pass *(opens once ④ is done: stand up the DB/repo test harness → **composite-FK assignee invariant / ADR-008 first** → then the deferred SQL tests: cascade, rebalance, visibility filter, card_count)*.
