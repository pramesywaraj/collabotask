...
- **Queue (in order):** ① P0 fixes ✅ → ② fractional positioning ✅ → ③ REST features ✅ → ③.5 auth httpOnly-cookie ✅ → **④ WebSocket + participation broadcast** *(A/B/C/D built + reviewed ✅ — D 2026-08-20: continuous enforcement + `ACCESS_REVOKED`; **Part E next**: workspace cascade fan-out + `EvictUserFromRooms`)* → ⑤ post-Phase-1 integrity pass *(opens once ④ is done: stand up the DB/repo test harness → **composite-FK assignee invariant / ADR-010 first** → then the deferred SQL tests: cascade, rebalance, visibility filter, card_count)*.
