...
| **B — `/ws` endpoint + lifecycle** | HTTP handler → hub; cookie auth, origin, `ResponseController` timeout fix; `JOIN_BOARD`/`LEAVE_BOARD`; `ACTIVE_USERS`/`USER_JOINED`/`USER_LEFT`; ping/pong | A real client connects, joins a board, sees live presence, survives reconnect. **Doc written + drift-reviewed ✅ 2026-07-31** (`/grill-with-docs`). **Code built + two-axis `/code-review` ✅ 2026-08-06** — production code spec-faithful + `build`/`vet` clean, all Q1–Q5 decisions verified; **⚠️ P0 blocker: Part B tests unwritten** (config/ws_socket/presence-handler cases) + P1 quick items (Swagger on `/ws`, `go mod tidy`, nil-ctx comment). Full findings + fixes + action plan → [part-b-ws-endpoint-lifecycle.md §Code review](./part-b-ws-endpoint-lifecycle.md) | A |
