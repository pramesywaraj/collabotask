...
| **B — `/ws` endpoint + lifecycle** | HTTP handler → hub; cookie auth, origin, `ResponseController` timeout fix; `JOIN_BOARD`/`LEAVE_BOARD`; `ACTIVE_USERS`/`USER_JOINED`/`USER_LEFT`; ping/pong | A real client connects, joins a board, sees live presence, survives reconnect. **Doc written + drift-reviewed ✅ 2026-07-31** (`/grill-with-docs`). **Code built + reviewed twice ✅ 2026-08-06** — two-axis `/code-review` (spec-faithful, `build`/`vet` clean, all Q1–Q5 verified), findings resolved, then **re-reviewed**: P0 tests written (realtime 23 / handler 9 / config 15) + all P1 items done (Swagger, `go mod tidy`, nil-ctx comment). **One optional test gap left: disconnect/readPump-EOF handler test (R1)** — behaviour is hub-covered, so non-blocking. Full findings + action plan → [part-b §Code review #2](./part-b-ws-endpoint-lifecycle.md#code-review-2--resolution-re-review-2026-08-06) | A |
| **C — Broadcaster port + mutation broadcasts** | Port + typed events + envelope adapter; inject into card/column/board usecases; `Broadcast` next to each `WriteActivity` (12 sites / 13 frame types); best-effort | **Built + reviewed ✅ 2026-08-20** — 3-way `/code-review` (two-axis + task-agent + spec-planner) aligned; fixes applied (P4 full-roster-row `MEMBER_ADDED.user` + SRS §5.2, table-driven tests, P1/P2 silence cases, S3 helper); `build`/`vet` clean, 569 tests. Manual two-client smoke still pending (local DB-env friction, not a code issue). In PR → [part-c](./part-c-broadcaster-mutations.md#aligned-code-review-2026-08-12) | A, B |
| **D — Continuous enforcement** | `ACCESS_REVOKED`; `EvictUser` on board-remove (UC-12d), `EvictExcept` on →PRIVATE flip (UC-12b), evict on board-leave (UC-10) | **Built + reviewed ✅ 2026-08-20** — 584 tests pass, `go test -race` clean; `/code-review` two-axis (0 hard-standards violations; P0 spec gap fixed: `CARD_UPDATED` also emitted on voluntary leave); evict-first ordering, `BroadcastToUser` + `ACCESS_REVOKED` before teardown, race fix in `TestUpdateBoard`. → [part-d](./part-d-continuous-enforcement.md) | A, C |
| **E — Participation cascade fan-out** | `WorkspaceUseCase` gains `Broadcaster`; UC-06/06c per-affected-board fan-out (evict → `MEMBER_REMOVED` → `CARD_UPDATED`) + `EvictUserFromRooms` leak closure | **Built + reviewed ✅ 2026-08-21** — 596 tests pass, `go test -race` clean; two-axis `/code-review` (0 P0/P1, 5/5 spec invariants); 3 P2s all resolved (P1-doc fallback log, S1 shared `common.BroadcastClearedCards`, S2 test-stub helper). New repo method `GetBoardIDsByWorkspace` (E1); `AffectedCard.BoardID` routing (E2); `EvictReasonRemovedFromWorkspace` (E3). **Step ④ COMPLETE — ⑤ opens.** → [part-e](./part-e-participation-cascade.md) | A, C, D |
...
- **Hub lock granularity → shard the `RWMutex`** (within one instance). The hub is guarded by a *single global* `sync.RWMutex` for *all* rooms, so board A's join/leave/evict serializes against board B's broadcasts. It holds up at Phase-1 scale — broadcasts take `RLock` (concurrent), critical sections are tiny map ops, and the lock is never held during a socket write (the send-channel + `writePump` keep I/O outside it). If join/evict *churn* ever makes it hot (mass-reconnect storms; note a pending writer also stalls new `RLock` readers), partition rooms across N mutexes by `boardID` hash — contention drops ~N-fold, contained to the realtime package. This is the *within-instance* step **before** the Redis multi-instance step below. *(Recorded 2026-07-31; to be taken up in a dedicated performance review.)*
