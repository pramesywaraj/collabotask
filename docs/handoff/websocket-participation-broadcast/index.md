...
| **B — `/ws` endpoint + lifecycle** | HTTP handler → hub; cookie auth, origin, `ResponseController` timeout fix; `JOIN_BOARD`/`LEAVE_BOARD`; `ACTIVE_USERS`/`USER_JOINED`/`USER_LEFT`; ping/pong | A real client connects, joins a board, sees live presence, survives reconnect. **Doc written + drift-reviewed ✅ 2026-07-31** (`/grill-with-docs`: fixed module path, `CheckViewAccess` arity, `NewWSSocket` export, missing imports, providers.go placement; **standalone `ProvideHub` singleton**; bounded ping + DB-check ctx; silent-join + presence **client contract**) → [part-b-ws-endpoint-lifecycle.md](./part-b-ws-endpoint-lifecycle.md) | A |
