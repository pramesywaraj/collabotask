...
**Where it goes:** this is the headline candidate for the **post-Phase-1 integrity pass** — write it up as **`adr-007`** *at that time* (not now; it's a candidate, not a decision). See `[[post-phase1-integration-tests]]`.
...
- SRS **§9 deferred / integrity-pass note**: record the composite-FK invariant as the `adr-007` candidate.
